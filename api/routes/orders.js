...
router.get("/", (req, res, next) => {
    ...
    Orders.find().populate("product").select("quantity product _id").then(s => {
...
router.get("/:orderId", (req, res, next) => {
    ...
    Orders.findById(id).exec().then(order => {
        
        res.status(200).json({order , request: {
            type: "GET",
            url: "http://localhost:3000/orders"
        }})})
...
router.delete("/:orderId", (req, res, next) => {
    ...
    Orders.remove({_id:id}).then(doc => res.status(200).json({
